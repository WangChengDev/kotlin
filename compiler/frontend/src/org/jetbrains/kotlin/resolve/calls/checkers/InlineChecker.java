...
class InlineChecker implements CallChecker {
    ...
    private void checkVisibilityAndAccess(
        ...
        if (isEffectivelyPublicOrPublishedApiFunction && declarationDescriptor.getVisibility() == Visibilities.PROTECTED) {
            context.getTrace().report(Errors.PROTECTED_CALL_FROM_PUBLIC_INLINE.on(expression));
        }
    }
    ...
    private static boolean isEffectivelyPublicOrPublishedApi(@NotNull CallableDescriptor descriptor) {
        ...
        if(visibility.toVisibility() == Visibilities.INTERNAL) {
            DeclarationDescriptor current = descriptor;
            while (current != null && !(current instanceof PackageFragmentDescriptor)) {
                if (!current.getAnnotations().hasAnnotation(KotlinBuiltIns.FQ_NAMES.publishedApi)) {
                    return false;
                }
